...
public final class ObjectDefinition {
	...
	public static ObjectDefinition forID(int i) {
		...
		case 22472:
			...
			objectDef.name = "Eldrax Database";
			break;
			case 26811:
		...
		case 33393:
			...
			objectDef.name = "Eldrax Teleporter";
			objectDef.overheadTextHeight = 450;
			objectDef.overheadText = "Eldrax Teleporter";
			objectDef.overheadTextOpacity = 0.8f;
			objectDef.font = Client.instance.newSmallFont;
			objectDef.overheadColour = 0xff0000;
			objectDef.overheadShadow = 0xffff00;
		...
		case 23307:
			...
			objectDef.overheadTextHeight = 500;
	...
	public boolean method577(int i) {
		if (modelTypes == null) {
			...
			for (int k = 0; k < modelIds.length; k++)
				flag1 &= Model.modelIsFetched(modelIds[k] & 0xffff);
		...
		for (int j = 0; j < modelTypes.length; j++)
			if (modelTypes[j] == i)
				return Model.modelIsFetched(modelIds[j] & 0xffff);
	...
	public boolean method579() {
		...
		for (int i = 0; i < modelIds.length; i++)
			flag1 &= Model.modelIsFetched(modelIds[i] & 0xffff);
		return flag1;
	}
	...
	private Model method581(int j, int k, int l) {
		...
		Model model_3 = new Model(modifiedModelColors == null && modifiedTexture == null, Frame.method532(k),
		...
		while (l-- > 0)
			model_3.method473();
		this.applyTexturing(model_3, this.type);
